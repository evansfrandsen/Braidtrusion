...
N_layers=3 #0 for one fiber per Yarn
Vf=0.55 #Fiber Volume Fraction
Lfiber=0.5
NFiberperYarn = 3 #Number of fibers per Yarn
...
#for Polymer fibers
FiberRadiusBYPolym=np.sqrt(SmYBY/(NPolym*np.pi)) #Radius of each Fiber
ABYPolym=np.pi*FiberRadiusBYPolym**2
#Section of Fiber
m.TrussSection(area=ABYPolym, material='Material-1', name='fiberSectionBY_Polym')
...
#for Reinforcement fibers
FiberRadiusAxPolym=np.sqrt(SmYAX/(NPolym*np.pi)) #Radius of each Fiber
AAXPolym=np.pi*FiberRadiusAxPolym**2
#Section of Fiber
for i in range(len(AAXPolym)):
    m.TrussSection(area=AAXPolym[i], material='Material-1', name='fiberSectionAx_Polym_'+str(i))
...
for j in range(Nby):
    for l in index_Reinf: #for Reinforcement fibers
        ...
        M_FiberBY.SectionAssignment(offset=0.0, offsetField='', offsetType=MIDDLE_SURFACE, 
            region=Region(elements=M_FiberBY.elements[:]), sectionName='fiberSectionBY_Reinf', 
		    thicknessAssignment=FROM_SECTION)
    for l in index_Polym: #for Polymer fibers
        k=j*NFiberperYarn+l
        M_FiberBY=M_FibersBY.get("M_PartFiberBY-"+str(k))
        M_FiberBY.SectionAssignment(offset=0.0, offsetField='', offsetType=MIDDLE_SURFACE, 
            region=Region(elements=M_FiberBY.elements[:]), sectionName='fiberSectionBY_Polym', 
		    thicknessAssignment=FROM_SECTION)    
#for Axial Yarn fibers
for j in range(Nax):
    for l in index_Reinf: #for Reinforcement fibers
        ...
        M_FiberAx.SectionAssignment(offset=0.0, offsetField='', offsetType=MIDDLE_SURFACE, 
                region=Region(elements=M_FiberAx.elements[:]), sectionName='fiberSectionAx_Reinf_'+str(j), 
		        thicknessAssignment=FROM_SECTION)
    for l in index_Polym: #for Polymer fibers
        k=j*NFiberperYarn+l
        M_FiberAx=M_FibersAx.get("M_PartFiberAx-"+str(k))
        M_FiberAx.SectionAssignment(offset=0.0, offsetField='', offsetType=MIDDLE_SURFACE, 
            region=Region(elements=M_FiberAx.elements[:]), sectionName='fiberSectionAx_Polym_'+str(j), 
            thicknessAssignment=FROM_SECTION)
