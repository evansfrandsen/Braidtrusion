...
class Braid :
    ...
    def __init__(self, Mandrel_Diameter, N_HG, N_slot,                                             # Braider Properties
     ...
     self.N_AX = N_AX
     self.Pitch = Pitch
     self.Type = N_BY/N_HG # Structure of the braid, 1 for diamond, 2 for regular
    ...
    def Init_AX(self):
        ...
        for k in range(self.N_AX): # For all axial yarns
            self.AX[k] = {'YARNa' : self.a_AX[k],
                        'YARNb' : self.b_AX[k],
                        'X' : 0,
                        'Y' : 0,
                        'Z' : np.linspace(Start, End, Num), 
                        'phi' : 2*np.pi*k/(self.N_HG),
                        'R' : 0 }
    ...
    def CircleSpacking(self,S,YarnCompaction):
        "Method that creates the points the points corresponding to each fiber in a Yarn"
        ...
        MaxCompaction = (NFiberperYarn)/(float((2 * N_layers + 1)**2)) # Maximun conpaction possible
    ...
    def Create_Wireframe_Model(self) :
            ...
            for i in self.SplineDataStruct['BY'].keys(): # looping through the different yarns
                ...
                for j in range(xx.shape[0]):
                    ...
                    self.EllipseCloud = self.CircleSpacking(S = self.Sf_BY + self.Sm_BY, YarnCompaction = self.Yarn_Compaction) # Draw the fibers

                    # we create a dictionnary cotaining the coordinates of x,y,z axis
            ...
            for i in self.SplineDataStruct['AX'].keys(): 
                ...
                for j in range(xx.shape[0]):
                    ...
                    Sf_AX=self.Sf_AX[i]
                    Sm_AX=self.Sm_AX[i]
                    self.EllipseCloud = self.CircleSpacking(S=Sf_AX+Sm_AX, YarnCompaction=self.Yarn_Compaction) # Draw the fibers 
    ...
    def Create_SplinePoints_Files(self,filename = 'it_0'):
            ...
            for i in range(N): #for each yarn
                for k in range(self.N_Fiber_Per_Yarn): #for each fiber                    
                    ...
                    for j in range(len(self.SplineDataStruct[yarnType][i]['X'])): #coordinates at each zone of each fiber
                        ...
                        globals()[f'{yarnType}_fiber'].write(f"{x:.10f} {y:.10f} {z:.10f}\n")                        
                # close file 
                globals()[f'{yarnType}_fiber'].close()                
        
        #print(f'The function was called {functioncallcounter} times')
        ...
        # First delete the old folder so that we do not have copies
        output_dir = Path('c:/Temp/Fiber_Points') # Desired location
        if output_dir.exists() and output_dir.is_dir(): # Check if it exists 
           shutil.rmtree(output_dir) # Delete it
           print('Deleted existing folder:', output_dir) # Confirm it was deleted
        output_dir.mkdir(parents = True)
        
        #Put all coordinate files in a single archive
        iterationFilePath = Path(iterationFilePath)
        for file in iterationFilePath.glob('*.txt'):
            shutil.copy(file,output_dir)
        print(f'Made new {output_dir} folder with new data')

        # Make zip file in the Splines folder in case it is needed for whatever reason
        archive_path = os.path.join(iterationFilePath.parent,'Fiber_Points')
        shutil.make_archive(str(archive_path),'zip', iterationFilePath)
        print(f'Archive made in code parent folder')
...
# Braiding properties 
Mandrel_Perimeter = 82.8e-3 #, changed for Abaqus Mandrel
Mandrel_Diameter = Mandrel_Perimeter/pi
Pitch = 50e-3
N_Pitch = 1
N_BY = 24 # Amount of braiding yarns
...
Ff_AX = np.array([2640,528,2640,528,2640,528,2640,528,2640,528,2640,528])*1e-6
Fm_AX = np.array([1520,304,1520,304,1520,304,1520,304,1520,304,1520,304])*1e-6
